 ...
 *
 * procedures for file categories
 */
export const file_upload = functions.storage
  .object()
  .onFinalize(async (object) => {
    const [uid, file_type] = object.name?.split("/") ?? [];

    switch (file_type) {
      case "sound": // TODO: add storage path constants
        await onSoundUpload(object);
        break;
    }
  });
 ...
 */
async function onSoundUpload(obj: functions.storage.ObjectMetadata) {
